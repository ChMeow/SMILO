...
PiCam = False
if(PiCam):
    resolutionX = 640  #larger the resolution slower the processing speed, but low resolution only works on near objects.
    resolutionY = 480
    camera = PiCamera()
...
while True:
        ...
        if(PiCam):
            if img in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
                frame = img.array
        else:
            frame = vs.read()
